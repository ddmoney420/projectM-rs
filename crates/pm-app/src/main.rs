...
use pm_render::wgpu;
use pm_render::{read_rgba8, Blit, GpuContext};
...
impl App {
    ...
    fn current_preset(&self) -> (Preset, String) {
        ...
        (Preset::load(BUILTIN_PRESET).expect("builtin preset"), "built-in".into())
    }
...
fn find_renderable(
    ...
    let mut engine = WarpEngine::new(ctx, Preset::load(BUILTIN_PRESET).unwrap(), w, h);
    warm_up(&mut engine, ctx, audio, frame_base);
